...
SUBROUTINE INITIAL
 ...
 REAL (KIND = 8) :: NX0,NY0,NX1,NY1,LLX,LLY,LL,XK,XK1,XK2
 REAL (KIND = 8), ALLOCATABLE :: XYZ_LOCAL(:,:),NODE_XYZ(:,:)
 ...
 TYPE (DY_POINT), ALLOCATABLE :: CELL_FACE(:),CELL(:),FACE(:)
 ...
 INTEGER :: SEC_NUM,INSIDE_FLAG
 REAL (KIND = 8) :: X_MAX,Y_MAX,X_SEC,X_MIN,Y_MIN
 REAL (KIND = 8) :: X_SEC_MAX,X_SEC_MIN
 REAL (KIND = 8) :: L_K(3)
 ...
 READ (1,*) LAMADA_S   ! STABILITY COEFFICIENT
 ...
 DO IK = 1,NODE_TOT
 	FACE(IK)%POINT_SIZE = 1
 	ALLOCATE (FACE(IK)%POINT_INDEX(1))
 	FACE(IK)%POINT_INDEX(1) = IK
 	READ (1,*) KK,NODE_XYZ(IK,1),NODE_XYZ(IK,2),FACE(IK)%POINT_BC,FACE(IK)%BC_NUM
 END DO
 CLOSE (1)
 
 OPEN (1,FILE = 'cell_vertex.dat')
 READ (1,*) CELL_TOT
...
! Reverse this order 
 ...
 OPEN (996,FILE = 'vertex_re.dat')
 OPEN (999,FILE = 'vertex_connect.dat')
 OPEN (997,FILE = 'cell_vertex_re.dat')
 WRITE (999,*) F_NN
 WRITE (996,*) F_NN
 WRITE (997,*) CELL_TOT
 DO IK = 1,FACE_TOT
 ...
 ! Recompute Output Coordinates
! DO I = 1,DRAW_NUM
! 	DO IK = 1,FACE_TOT
! 		NK = FC_NEW(IK)
! 		IF (NK.EQ.DRAW_XYZ(I)) THEN 
! 			DRAW_XYZ(I) = IK
! 		END IF
! 	END DO
! END DO
 ...
 DO K = 1,FACE_TOT
 	...
 	NK = LOCAL(1)
 	FACE_COE(K)%X = NODE_XYZ(NK,1)
 	FACE_COE(K)%Y = NODE_XYZ(NK,2)
 	LOCAL = 0
 END DO
 CLOSE (1)
 
 OPEN (1,FILE = 'cell_vertex_re.dat')
 READ (1,*) CELL_TOT
 ...
 OPEN (1,FILE = 'vertex_connect.dat')
 READ (1,*) FACE_TOT
 LOCAL = 0
 DO K = 1,FACE_TOT
 	READ (1,*) NK,(LOCAL(IK),IK=1,NK)
 ...
 DO K = 1,FACE_TOT
 	...
 	FACE_COE(K)%CELL_NUM = 0
 END DO
 
 ! Reorder the vertex neighbor number
 DO K = 1,FACE_TOT
 	NK = FACE_COE(K)%C_SIZE
 	DO JK = 2,NK-1
 		DO KK = JK+1,NK
 ...
 DO K = 1,CELL_TOT
 	...
 	DO IK = 1,NK
 		...
 		X1 = FACE_COE(KK)%X; Y1 = FACE_COE(KK)%Y
 		FACE_COE(KK)%CELL_NUM = FACE_COE(KK)%CELL_NUM+1
 		JK = FACE_COE(KK)%CELL_NUM
 		FACE_COE(KK)%CELL_IND(JK) = K
 		 
 		IF (IK.LT.NK) THEN 
 			KK = CELL_COE(K)%POINT_INDEX(IK+1)
 			X2 = FACE_COE(KK)%X; Y2 = FACE_COE(KK)%Y
 		ELSE 
 		...
 		CELL_COE(K)%AREA(IK) = LL
 		
 		NX1 = -NY; NY1 = NX
 		
 		CELL_COE(K)%NX(IK) = NX1; CELL_COE(K)%NY(IK) = NY1
 ...
 ALLOCATE (WELL_CELL_NUM(WELL_NUM))
...
! OPEN (993,FILE = 'CENTER_CAL.DAT')
 ...
 DO K = 1,CELL_TOT
 	...
 	END DO
 	NK = CELL_COE(K)%POINT_SIZE
 	ALLOCATE (XYZ_LOCAL(NK,2))
 	DO IK = 1,NK
 		KK = CELL_COE(K)%POINT_INDEX(IK)
 		XYZ_LOCAL(IK,1) = FACE_COE(KK)%X
 		XYZ_LOCAL(IK,2) = FACE_COE(KK)%Y
 	END DO
 	X0 = CELL_COE(K)%X; Y0 = CELL_COE(K)%Y
 	FACE_NUM = NK
 	CALL MFD_CAL(CELL_COE(K)%MCF,X0,Y0,FACE_NUM,XYZ_LOCAL,KTH,CELL_COE(K)%FA_COE,K_SCHEME, &
 	             CELL_COE(K)%RRNC,LAMADA_S,CELL_COE(K)%NX,CELL_COE(K)%NY)
 	CELL_COE(K)%SINGULAR_FLAG = 0
 	DO IK = 1,FACE_NUM
 		KK = CELL_COE(K)%POINT_INDEX(IK)
 		XYZ_LOCAL(IK,1) = FACE_COE(KK)%X
 		XYZ_LOCAL(IK,2) = FACE_COE(KK)%Y
 	END DO
 ! Find the well location
 	DO KK = 1,WELL_NUM
 		X0 = WELL_X(KK); Y0 = WELL_Y(KK)
 		X_MAX = MAXVAL(XYZ_LOCAL(:,1))
 		X_MAX = X_MAX+10.0
 		SEC_NUM = 0
 		INSIDE_FLAG = 0
 		DO IK = 1,FACE_NUM
 			X1 = XYZ_LOCAL(IK,1); Y1 = XYZ_LOCAL(IK,2)
 			XK = (X1-X0)**2+(Y1-Y0)**2
 			XK = SQRT(XK)
 			IF (XK.LT.1.0E-10) THEN 
 				INSIDE_FLAG = 1
 			END IF
 		END DO
 		IF (INSIDE_FLAG.EQ.0) THEN 
 			DO IK = 1,FACE_NUM
 				X1 = XYZ_LOCAL(IK,1); Y1 = XYZ_LOCAL(IK,2)
 				IF (IK.LT.FACE_NUM) THEN 
 					X2 = XYZ_LOCAL(IK+1,1); Y2 = XYZ_LOCAL(IK+1,2)
 				ELSE 
 					X2 = XYZ_LOCAL(1,1); Y2 = XYZ_LOCAL(1,2)
 				END IF
 				L_K(1) = Y2-Y1; L_K(2) = X1-X2
 				L_K(3) = L_K(1)*X1+L_K(2)*Y1
 				
 				IF (ABS(L_K(1)).LT.1.0E-10) THEN 
 					XK = ABS(Y0-Y2)
 					XK1 = (X0-X1)*(X0-X2)
 					IF (XK.LT.1.0E-10.AND.XK1.LE.0.0) THEN 
 						INSIDE_FLAG = 1
 					END IF 
 				ELSE 
 					X_SEC = (L_K(3)-L_K(2)*Y0)/L_K(1)
 					XK1 = (Y0-Y1)*(Y0-Y2)
 					XK2 = (X_SEC-X0)*(X_SEC-X_MAX)
 					IF (ABS(XK2).LT.1.0E-10) THEN 
 						INSIDE_FLAG = 1
 					END IF
 					XK = ABS(Y0-Y1)
 					IF (XK.LT.1.0E-10) THEN 
 						SEC_NUM = SEC_NUM+1
 					END IF
 					IF (XK1.LT.0.0.AND.XK2.LT.0.0) THEN 
 						SEC_NUM = SEC_NUM+1
 					END IF
 				END IF 
 			END DO
 		END IF
 		IF (INSIDE_FLAG.EQ.0) THEN 
 			IF (MOD(SEC_NUM,2).NE.0) THEN 
 				INSIDE_FLAG = 1
 			END IF
 		END IF
 		IF (INSIDE_FLAG.EQ.1) THEN 
 			WELL_CELL_NUM(KK) = WELL_CELL_NUM(KK)+1
	 		CELL_COE(K)%SINGULAR_FLAG = KK 
	 		WRITE (*,*) 'WELL',KK,K,WELL_CELL_NUM(KK)
 			ALLOCATE (CELL_COE(K)%SHAPE_COE(FACE_NUM))
 	! COMPUTE GMLS SHAPE FUNCTIONS (THE MODULE IS IN solver_mfd.f95)
 			X0 = WELL_X(KK); Y0 = WELL_Y(KK)
 		!	JK = 3
 		!	CALL GMLS(X0,Y0,FACE_NUM,JK,XYZ_LOCAL,CELL_COE(K)%SHAPE_COE)
 			JK = FACE_NUM+3
 			CALL RBF_GAU(X0,Y0,FACE_NUM,JK,XYZ_LOCAL,CELL_COE(K)%SHAPE_COE)
 		END IF
 	END DO
...
! CLOSE (993)
 ...
 OPEN (99,FILE = 'WELL_LOCATION.DAT')
 DO IK = 1,CELL_TOT
 	IF (CELL_COE(IK)%SINGULAR_FLAG.NE.0) THEN 
 		WRITE (99,*) 'WELL_NUM = ',CELL_COE(IK)%SINGULAR_FLAG, &
 		QWELL(CELL_COE(IK)%SINGULAR_FLAG)
 		WRITE (99,*) 'CENTER_LOCATION = ',CELL_COE(IK)%X,CELL_COE(IK)%Y
 		WRITE (99,*) 'SHAPE_COE = '
 		FACE_NUM = CELL_COE(IK)%POINT_SIZE
 		WRITE (99,'(20F20.10)') (CELL_COE(IK)%SHAPE_COE(JK),JK=1,FACE_NUM)
 	END IF
 END DO
 ...
 IF (CELL_COE(K)%POINT_TYPE.NE.(-1)) THEN
 	...
 	DO IK = 1,FACE_NUM
 		KK = CELL_COE(K)%POINT_INDEX(IK)
 		IF (UIT.EQ.1) THEN 
 			call vg(CELL_COE(K)%MTH,KTH1,CELL_COE(K)%CTH,FACE_COE(KK)%P-FACE_COE(KK)%Y, &
 			CELL_COE(K)%ALPHA,CELL_COE(K)%THETAS,CELL_COE(K)%THETAR,CELL_COE(K)%K_SS,CELL_COE(K)%M2)
 		END IF
 		IF (UIT.EQ.2) THEN 
 			call Haverkamp(CELL_COE(K)%MTH,KTH1,CELL_COE(K)%CTH,FACE_COE(KK)%P-FACE_COE(KK)%Y,&
 			CELL_COE(K)%ALPHA,CELL_COE(K)%THETAS,CELL_COE(K)%THETAR,CELL_COE(K)%K_SS,CELL_COE(K)%A,CELL_COE(K)%M2,CELL_COE(K)%B)
 		END IF
 		IF (UIT.EQ.3) THEN 
 			call gardner(CELL_COE(K)%MTH,KTH1,CELL_COE(K)%CTH,FACE_COE(KK)%P-FACE_COE(KK)%Y,&
 			CELL_COE(K)%ALPHA,CELL_COE(K)%THETAS,CELL_COE(K)%THETAR,CELL_COE(K)%K_SS,CELL_COE(K)%M2)
 		END IF
